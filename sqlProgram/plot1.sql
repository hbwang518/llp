...
from AB a join ABBag b on a.[样地编号]=b.[样地编号]
...
update AB set [总]
select a.autoID,a.EnvFlag 
from AB a 
where ltrim(rtrim(a.EnvFlag)) like  '%大%'
...
update AB set [样品鲜重] = a.[样品鲜重] from AB join AB$ a on AB.autoID=a.autoID
...
--update AB set speciesSeq=left([物种],

select [物种] from AB
select replace([物种],[0x4e00,0x9fa5],'') from AB

select * from AB order by 

select  * from AB order by [物种]

select* from AB where envFlag like '%大%'
select * into grassland2.dbo.ABBag from ABbag 
select * into grassland2.dbo.AB from AB
...
update soil set [净鲜重]=[总鲜重]-a.[土壤袋重量]-[砾石干重（g)]-isnull(BB.[净鲜重],BB.[净风干重])
from soil join soilBBbag a on soil.[样地编号]=a.[样地编号]
join BB on soil.[样地编号]=BB.[样地编号] and soil.[样品号]=BB.[样品号]

update soil set [净风干重]=[风干重]-[砾石干重（g)]-isnull(BB.[净风干重],BB.[净鲜重])
from soil join soilBBbag a on soil.[样地编号]=a.[样地编号]
join BB on soil.[样地编号]=BB.[样地编号] and soil.[样品号]=BB.[样品号]

update [样地草本] set latinName = b.latinName from [样地草本] a join [标本] b on a.[样地编号]=b.[样地号] and a.cnName=b.cnName
