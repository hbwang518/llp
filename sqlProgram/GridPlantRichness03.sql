...
select b.Id, a.plant_id, a.county_code,a.lowAltitude PlantLowAltitude,a.highAltitude PlantHighAltitude,b.altitude_min GridAltitudeMin,b.altitude_max GridAltitudeMax into PlantCountyAltitudeResult03
from distinctPlantCounty02 a ,Grid03 b, m_plant c
...
select * into PlantCountyAltitudeValidResult03 from PlantCountyAltitudeResult03 
where 
...
CREATE INDEX PlantCountyAltitudeValidResult03_Plant_id ON PlantCountyAltitudeValidResult03 (plant_id)
CREATE INDEX PlantCountyAltitudeValidResult03_Id ON PlantCountyAltitudeValidResult03 (Id)
CREATE INDEX plant_index  ON M_Plant  (id)

select Id,count(distinct(plant_id)) Species,count(distinct(cnFamily)) Family,count(distinct(cnGenus)) Genus into a0  from (select a.Id,b.id plant_id,b.cnFamily,b.cnGenus from PlantCountyAltitudeValidResult03 a,M_Plant b where a.plant_id=b.id) w group by w.Id 
select Id,count(distinct(plant_id)) Species12,count(distinct(cnFamily)) Family12,count(distinct(cnGenus)) Genus12 into a1  from (select a.Id,b.id plant_id,b.cnFamily,b.cnGenus from PlantCountyAltitudeValidResult03 a,M_Plant b where a.plant_id=b.id and (b.yearCode ='2' or b.yearCode ='1')) w group by w.Id 
select Id,count(distinct(plant_id)) Species3,count(distinct(cnFamily)) Family3,count(distinct(cnGenus)) Genus3 into a2  from (select a.Id,b.id plant_id,b.cnFamily,b.cnGenus from PlantCountyAltitudeValidResult03 a,M_Plant b where a.plant_id=b.id and b.yearCode ='3') w group by w.Id 
select Id,count(distinct(plant_id)) SpeciesWoody,count(distinct(cnFamily)) FamilyWoody,count(distinct(cnGenus)) GenusWoody into a3  from (select a.Id,b.id plant_id,b.cnFamily,b.cnGenus from PlantCountyAltitudeValidResult03 a,M_Plant b where a.plant_id=b.id and (b.typeCode ='1' or b.typeCode ='3')) w group by w.Id 
select Id,count(distinct(plant_id)) SpeciesHerb,count(distinct(cnFamily)) FamilyHerb,count(distinct(cnGenus)) GenusHerb into a4  from (select a.Id,b.id plant_id,b.cnFamily,b.cnGenus from PlantCountyAltitudeValidResult03 a,M_Plant b where a.plant_id=b.id and (b.typeCode ='2' or b.typeCode ='4')) w group by w.Id
select Id,count(distinct(plant_id)) SpeciesFern,count(distinct(cnFamily)) FamilyFern,count(distinct(cnGenus)) GenusFern into a5  from (select a.Id,b.id plant_id,b.cnFamily,b.cnGenus from PlantCountyAltitudeValidResult03 a,M_Plant b where a.plant_id=b.id and b.menCode ='1') w group by w.Id 
select Id,count(distinct(plant_id)) SpeciesGym,count(distinct(cnFamily)) FamilyGym,count(distinct(cnGenus)) GenusGym into a6  from (select a.Id,b.id plant_id,b.cnFamily,b.cnGenus from PlantCountyAltitudeValidResult03 a,M_Plant b where a.plant_id=b.id and b.menCode ='2') w group by w.Id 
select Id,count(distinct(plant_id)) SpeciesAng,count(distinct(cnFamily)) FamilyAng,count(distinct(cnGenus)) GenusAng into a7  from (select a.Id,b.id plant_id,b.cnFamily,b.cnGenus from PlantCountyAltitudeValidResult03 a,M_Plant b where a.plant_id=b.id and b.menCode ='3') w group by w.Id 
select Id,count(distinct(plant_id)) SpeciesDan,count(distinct(cnFamily)) FamilyDan,count(distinct(cnGenus)) GenusDan into a8 from (select a.Id,b.id plant_id,b.cnFamily,b.cnGenus from PlantCountyAltitudeValidResult03 a,M_Plant b where a.plant_id=b.id and b.endangeredFlag ='1') w group by w.Id 
select Id,count(distinct(plant_id)) SpeciesEph,count(distinct(cnFamily)) FamilyEph,count(distinct(cnGenus)) GenusEph into a9 from (select a.Id,b.id plant_id,b.cnFamily,b.cnGenus from PlantCountyAltitudeValidResult03 a,M_Plant b where a.plant_id=b.id and b.ephemeralFlag ='1') w group by w.Id 
select Id,count(distinct(plant_id)) SpeciesEnd,count(distinct(cnFamily)) FamilyEnd,count(distinct(cnGenus)) GenusEnd into a10 from (select a.Id,b.id plant_id,b.cnFamily,b.cnGenus from PlantCountyAltitudeValidResult03 a,M_Plant b where a.plant_id=b.id and b.endemismFlag ='1') w group by w.Id 
select Id,count(distinct(plant_id)) SpeciesXJDan,count(distinct(cnFamily)) FamilyXJDan ,count(distinct(cnGenus)) GenusXJDan into a11 from (select a.Id,b.id plant_id,b.cnFamily,b.cnGenus from PlantCountyAltitudeValidResult03 a,M_Plant b where a.plant_id=b.id and b.xjEndangeredFlag ='1') w group by w.Id 
select Id,count(distinct(plant_id)) SpeciesHal,count(distinct(cnFamily)) FamilyHal,count(distinct(cnGenus)) GenusHal into a12 from (select a.Id,b.id plant_id,b.cnFamily,b.cnGenus from PlantCountyAltitudeValidResult03 a,M_Plant b where a.plant_id=b.id and b.halophilousFlag ='1') w group by w.Id 
...
into GridSpecies03 from Grid03 z	
